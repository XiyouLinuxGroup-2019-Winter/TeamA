...
/*=============================================================*/
/**/const char *g_host_name = "Linux_8853";
/**/const char *g_user_name = "18149308853";
/**/const char *g_passwd = "123";
/**/const char *g_db_name = "chat_room_8853";
/*=============================================================*/
...
void init_serv()
{
    ...
    //绑定地址
    if (bind(connfd, (struct sockaddr *)&sockAddr, sizeof(sockAddr)) < 0) {
        printf("bind address fail.\n");
        exit(-1);
    }
    //开始监听新加入的连接
    if (listen(connfd, CONNECTION_PENDING_LIMIT) < 0) {
    ...
    while (1) {
        ...
        for (i = 0; i < epoll_events_count; i++) {
            ...
            printf("socket fd:%d\n", sfd);
            //创建的socket connfd 上有新的事件（新的客户端连接）
            if (sfd == connfd) {
                //接受客户端连接
                client_addr_length = sizeof(struct sockaddr_in);
                printf("client addr length:%d\n", client_addr_length);
                clientFd = accept(connfd, (struct sockaddr *)&clientAddr, &client_addr_length);
